...
| `agent-fleet task race <N> "<prompt>" [--repo <dir>] [--name <name>] [--isolation <rung>] [--focus]` | Best-of-N: spawn N attempts at one intent, each an isolated worktree task (own branch, pane, record) sharing a race id; the rail, picker and inbox badge them `⑂k/N`. Prints the race id and each `<task-id> <pane-id>`. Needs a git repo; the `host` rung is raised to `worktree`. |
...
| `agent-fleet task intent <id\|%pane> "<text>"` | Set a task's intent by hand; the rail and picker title the row with it. Renaming a tab (`Prefix T`) updates an adopted agent's placeholder intent automatically. |
| `agent-fleet task adopt [%pane] ["intent"]` | Bring an existing pane into the record loop (intent defaults to the window name). Hooked agents auto-adopt on their first event; adopt covers idle agents and panes from before the record system. |
...
| `agent-fleet review --race <rid> [--pick <k\|id>] [--keep-branches]` | Compare a race's attempts side by side (state, branch, commits ahead of the merge-base, diff size, uncommitted work); on a TTY, prompt for the winner. `--pick` decides: the winner gets the atomic finish (merge, close, reclaim), the others are marked abandoned, their panes closed and their worktrees and branches reclaimed. `task history` shows the race as one entry (`⑂N attempts`). |
...
| `agent-fleet answer <id\|%pane> approve\|deny\|text "<reply>" [--fp <sum>]` | Guarded keystrokes into a **waiting** agent: Enter, Escape, or a typed reply. `--fp` (from `answer <pane> fp`) refuses when the pane changed since the caller looked, so a reply never lands in a prompt nobody saw. `answer --all approve [--yes]` clears every local wait, confirming first. Refusals exit 1, nothing sent. The inbox's `^y`/`^n`/`^t`/`^a` call this. |
...
| `Prefix ?` | Fleet keymap as a menu — every row also launches its action (tmux's raw dump stays at `:list-keys`) |
