...
check "12s @ interval=15 -> healthy" "! fleet_rows | grep -q 'snapshot stale'"

printf 'T %s 15\nA a|@1|1|w|%%5|claude|wait\n' "$(( now - 90 ))" > "$SNAPDIR/fleet.snapshot"
check "90s @ interval=15 -> stale banner" "fleet_rows | grep -q 'snapshot stale'"

printf 'T %s\nA a|@1|1|w|%%5|claude|wait\n' "$(( now - 12 ))" > "$SNAPDIR/fleet.snapshot"
check "legacy T (no interval) -> stale at 12s" "fleet_rows | grep -q 'snapshot stale'"
